...
def build_segmentation_model(input_shape: tuple[int, int, int]=(*IMG_SIZE, len(MODALITIES)), num_seg_classes: int=NUM_CLASSES) -> Model:
    ...
    seg_output = layers.Conv2D(num_seg_classes, kernel_size=1, activation='softmax', name='segmentation')(c6) # 'softmax': raw output scores (logits) -> probability dist
    ...
    model.compile(
        ...
        loss=lambda y_true, y_pred: dice_loss(y_true, y_pred) + CategoricalCrossentropy()(y_true, y_pred), # how to measure error (spatial overlap + class prob error)
